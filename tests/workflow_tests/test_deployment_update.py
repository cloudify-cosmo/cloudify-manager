...
class TestDeploymentUpdate(TestCase):
    ...
    def _test_add_node(self, archive_mode=False):
        """
        Helper method which can run the test both in archive mode and in
        blueprint mode
        :param archive_mode: whether to archive the blueprint before the test
        :return:
        """
        estimated_number_of_instance = 3
        ...
        try:
            ...
            self.assertEquals(estimated_number_of_instance,
                              len(added_instances))
            ...
            # Checking that each node instance was indeed created with
            # the proper relationships and ran all of the operations
    ...
    def test_add_node_bp(self):
        ...
        adding through the stage blueprint api.
    ...
    def test_add_node_archive(self):
        """
        add a node (type exists) which is contained in an existing node
        adding through the stage archive api.
        - assert that both node and node instance have been created
        - assert the node/instance relationships have been created
        - assert the 'update' workflow has been executed and
          all related operations were executed as well
