...
workflows:
    wf1:
        mapping: file:///dev/null
        availability_rules:
            node_instances_active: {active}
"""
        base_bp_path = \
            self.make_yaml_file(bp_template.format(active='none'))
        some_active_bp_path = \
            self.make_yaml_file(bp_template.format(active='some'))
        all_active_bp_path = \
            self.make_yaml_file(bp_template.format(active='all'))
        deployment, _ = self.deploy_application(base_bp_path)

        self.client.blueprints.upload(some_active_bp_path, 'bp-some')
        self.client.blueprints.upload(all_active_bp_path, 'bp-all')
        wait_for_blueprint_upload('bp-some', self.client)
        wait_for_blueprint_upload('bp-all', self.client)
        with self.assertRaises(CloudifyClientError) as cm:
            self.execute_workflow('wf1', deployment.id)
        assert cm.exception.error_code == 'unavailable_workflow_error'
        self._wait_for_update(self._do_update(deployment.id, 'bp-all'))
        self.execute_workflow('wf1', deployment.id)  # doesn't throw
        ...
        self.client.node_instances.update(
            active_node_instance_ids[0], state='qwerty', force=True)
        with self.assertRaises(CloudifyClientError) as cm:
            self.execute_workflow('wf1', deployment.id)
        assert cm.exception.error_code == 'unavailable_workflow_error'
        self._wait_for_update(self._do_update(deployment.id, 'bp-some'))
        self.execute_workflow('wf1', deployment.id)  # doesn't throw
