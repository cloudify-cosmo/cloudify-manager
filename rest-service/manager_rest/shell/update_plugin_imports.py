...
def spec_from_url(resolver: ResolverWithCatalogSupport, url: str) -> tuple:
    ...
    for _, spec in plugin_yaml.get("plugins", {}).items():
        if spec.get(PLUGIN_PACKAGE_NAME) and spec.get(PLUGIN_PACKAGE_VERSION):
            return spec[PLUGIN_PACKAGE_NAME], spec[PLUGIN_PACKAGE_VERSION]
    return None, None
...
def spec_from_source(import_line):
    ...
    try:
        ...
        return plugin_spec.get("package_name"), plugin_spec.get(
            "package_version"
        )
    except Exception:
        return None, None
...
def plugin_spec(
    resolver: ResolverWithCatalogSupport, import_line: str
) -> tuple:
    if import_line.startswith("http://") or import_line.startswith("https://"):
...
def plugins_in_a_plan(plan: Plan) -> collections.Iterable:
    for executor in [
        ...
        for plugin in plan[executor]:
            if plugin[PLUGIN_PACKAGE_NAME] and plugin[PLUGIN_PACKAGE_VERSION]:
                yield plugin
...
def scan_blueprint(
    ...
    current_import_from_stats: Dict[str, int] = collections.defaultdict(
        lambda: 0
    )
    suggested_import_from_stats: Dict[str, int] = collections.defaultdict(
        lambda: 0
    )
    pinned_stats = {
        CURRENT_IS_PINNED: 0,
    ...
    for import_line in imports:
        ...
        update_stats(
            suggested_import_from=IMPORT_FROM_MANAGED,
...
def write_updated_blueprint(
    input_file_name: str, output_file_name: str, import_updates: list
):
    try:
            ...
            with open(output_file_name, "wb") as output_file:
                for update in import_updates:
                    content = input_file.read(
                        update[START_POS] - input_file.tell()
                    )
                    output_file.write(content)
                    output_file.write(
                        update[REPLACEMENT].encode("utf-8", "ignore")
                    )
...
def correct_blueprint(
    blueprint: models.Blueprint, plugin_names: tuple, mappings: Dict[str, List]
) -> str:
    ...
    common.write_blueprint_diff(
        file_name, new_file_name, common.blueprint_diff_file_name(blueprint)
    )
...
def printout_scanning_stats(
    total_blueprints: int, mappings: dict, stats: dict
):
    number_of_unknown_or_updates = len(
        [b for b in mappings.values() if UPDATES in b or UNKNOWN in b]
    )
    number_of_unknown = len([b for b in mappings.values() if UNKNOWN in b])
    pinned = (
        sum(s.get(CURRENT_IS_PINNED) for s in stats.values()),
        sum(s.get(SUGGESTED_IS_PINNED) for s in stats.values()),
    )
    url_import = (
        sum(
            s.get(CURRENT_IMPORT_FROM, {}).get(IMPORT_FROM_URL, 0)
            for s in stats.values()
        ),
        sum(
            s.get(SUGGESTED_IMPORT_FROM, {}).get(IMPORT_FROM_URL, 0)
            for s in stats.values()
        ),
    )
    source_import = (
        sum(
            s.get(CURRENT_IMPORT_FROM, {}).get(IMPORT_FROM_SOURCE, 0)
            for s in stats.values()
        ),
        sum(
            s.get(SUGGESTED_IMPORT_FROM, {}).get(IMPORT_FROM_SOURCE, 0)
            for s in stats.values()
        ),
    )
    print("\n\n                             SCANNING STATS")
    print("----------------------------------------------+---------------")
    print(
        " Number blueprints scanned                    | {0:14d}".format(
            total_blueprints
        )
    )
    print("                                              +---------------")
    ...
    print("----------------------------------------------+--------+------")
    print(
        " Number of blueprints with one or more issues | {0:6d} | {1:5d}"
        .format(number_of_unknown_or_updates, number_of_unknown)
    )
    print(
        " Number of blueprints with version lock       | {0:6d} | {1:5d}"
        .format(pinned[0], pinned[1])
    )
    print(
        " Number of blueprints with URL import         | {0:6d} | {1:5d}"
        .format(url_import[0], url_import[1])
    )
    print(
        " Number of blueprints with source import      | {0:6d} | {1:5d}"
...
def printout_correction_stats(stats):
    ...
    print("----------------------------------------------+-------")
    print(
        " Number blueprints scanned                    | {0:5d}".format(
            total_blueprints_scanned
        )
    )
    print("----------------------------------------------+-------")
    print(
        " Number of blueprints corrected               | {0:5d}".format(
            len(stats.get(UPDATES, []))
        )
    )
    print(
        " Number of blueprints that are valid          | {0:5d}".format(
            len(stats.get(UPDATES, [])) + len(stats.get(FINE, []))
        )
    )
    print(
        " Number of blueprints that require attention  | {0:5d}".format(
            number_of_unknown
        )
    )
    if number_of_unknown:
        ...
        if number_of_unknown < 20:
            print(
                "These blueprints require manual attention: {0}".format(
...
@click.option(
    ...
    "plugin_names",
    multiple=True,
    help="Plugin(s) to update (you can provide " "multiple --plugin-name(s).",
)
@click.option(
    "-b",
    "--blueprint",
    "blueprint_ids",
    multiple=True,
    help="Blueprint(s) to update (you can provide " "multiple --blueprint(s).",
)
@click.option(
    "--mapping",
    "mapping_file",
...
def main(
    ...
    if all_tenants and tenant_names:
        logger.critical(
            "--all-tenants and --tenant-name options are " "mutually exclusive"
        )
        sys.exit(1)
    if not tenant_names:
    ...
    for tenant in tenants:
        ...
        for blueprint in blueprints:
            ...
            if correct:
                logger.info(
                    "Correcting tenant's `%s` blueprint `%s`",
                ...
                logger.info(
                    "Scanning tenant's `%s` blueprint `%s`",
                    blueprint.tenant.name,
                    blueprint.id,
                )
                try:
                    a_mapping, a_statistic, a_suggestion = scan_blueprint(
                        resolver, blueprint, plugin_names
                    )
                except common.UpdateException as ex:
                    logger.error(
