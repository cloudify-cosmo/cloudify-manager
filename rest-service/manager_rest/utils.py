...
def extract_blueprint_archive_to_mgr(destination_root, archive_path):
    """
...
def save_request_content_to_file(request, archive_target_path, data_kind,
                                 url_key):
    """
    ...
    :param data_kind: the kind of the data (e.g. 'blueprint')
    ...
    if url_key in request.args:
        ...
        if not request.data:
            raise manager_exceptions.BadParametersError(
                    ...
                    '"{1}" in query parameters'.format(data_kind,
                                                       url_key))
