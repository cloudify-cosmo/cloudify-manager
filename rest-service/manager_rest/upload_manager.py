...
class UploadARIAServiceTemplateManager(UploadedBlueprintsManager):
    ...
    def __init__(self, core, *args, **kwargs):
        super(UploadARIAServiceTemplateManager, self).__init__(*args, **kwargs)
        self._core = core

    def _get_data_url_key(self):
        return 'service_template_csar_url'
    ...
    def _prepare_and_process_doc(
        ...
        base_dir = os.path.join(file_server_root, application_dir)
        service_template_path = os.path.join(base_dir,
                                             application_file_name)
        self._core.create_service_template(
            service_template_name=data_id,
            service_template_dir=base_dir,
            service_template_path=service_template_path,
        )
        return \
            self._core.model_storage.service_template.get_by_name(data_id), \
            application_file_name
