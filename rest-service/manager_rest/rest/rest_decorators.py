...
class marshal_with(object):
    def __init__(self, response_class):
        ...
        """
        notset = object()
        if getattr(response_class, 'response_fields', notset) is not notset:
            self._fields = response_class.response_fields
        elif getattr(response_class, 'resource_fields', notset) is not notset:
            self._fields = response_class.resource_fields
        else:
            raise RuntimeError(
                'Response class {0} does not contain a "resource_fields" '
                'class variable'.format(response_class))
