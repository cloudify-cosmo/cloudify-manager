...
class Deployment(CreatedAtMixin, SQLResourceBase):
    ...
    def evaluate_deployment_status(self, exclude_sub_deployments=False):
        ...
        """
        if self.latest_execution_status == DeploymentState.COMPLETED \
                and self.installation_status == DeploymentState.INACTIVE:
            # This is a special-case expected to be hit when create_dep_env
            # has finished but nothing else has happened.
            deployment_status = DeploymentState.IN_PROGRESS
        elif self.latest_execution_status == DeploymentState.IN_PROGRESS:
            deployment_status = DeploymentState.IN_PROGRESS
