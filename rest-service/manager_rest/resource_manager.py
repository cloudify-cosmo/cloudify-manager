...
class ResourceManager(object):
    ...
    def _is_valid_yaml_file(self, plugin):
        ...
        if not plugin_yaml.get(constants.PLUGINS, {}):
            raise manager_exceptions.InvalidPluginError(
                'YAML file must be in correct plugin format.'
            )
    ...
    def install_plugin(self, plugin):
        ...
        if plugin_yaml_path:
            if self._is_valid_yaml_file(plugin):
                if not self._is_central_executor_plugin(plugin):
                    return
