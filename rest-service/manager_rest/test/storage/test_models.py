...
from . import ModelsIntegrationTestsBase
...
class ModelsIntegrationTests(ModelsIntegrationTestsBase):
    @patch.object(storage_manager.SQLStorageManager, '_add_tenant_filter', mock_filter)
    @patch.object(storage_manager.SQLStorageManager, '_add_permissions_filter', mock_filter)
    ...
    @patch.object(storage_manager.SQLStorageManager, '_add_tenant_filter', mock_filter)
    @patch.object(storage_manager.SQLStorageManager, '_add_permissions_filter', mock_filter)
    @patch.object(storage_manager.SQLStorageManager, '_associate_users_and_tenants', lambda *args: None)
    @patch.object(storage_manager.SQLStorageManager, '_validate_unique_resource_id_per_tenant', lambda *args: None)
    def test_model_relationship(self):
        ...
        self.assertEquals(self.sm.get(models.Blueprint, blueprint.id),
