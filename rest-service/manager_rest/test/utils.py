...
class skip_execution_transition_validation(object):

    def __init__(self):
        self._original_valid_transition = \
            models.Execution.VALID_TRANSITIONS.copy()

    @staticmethod
    def _enter(new_transition):
        new_transition = new_transition or {
            k: set(models.Execution.STATES + [None]) - set(k)
    ...
    def _exit(self):
        models.Execution.VALID_TRANSITIONS.clear()
        models.Execution.VALID_TRANSITIONS.update(
            self._original_valid_transition)
