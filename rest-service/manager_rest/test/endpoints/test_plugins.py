...
class PluginsTest(BaseServerTestCase):
    ...
    @attr(client_min_version=3.1,
          client_max_version=base_test.LATEST_API_VERSION)
    def test_plugin_upload_with_empty_yaml(self):
        tmp_file_path = self.create_wheel('cloudify-script-plugin', '1.5.3')
        yaml_path = self.get_full_path('mock_blueprint/plugin_empty.yaml')
        zip_path = self.zip_files([tmp_file_path, yaml_path])
    ...
    @attr(client_min_version=3.1,
          client_max_version=base_test.LATEST_API_VERSION)
    def test_plugin_upload_with_bad_yaml(self):
        tmp_file_path = self.create_wheel('cloudify-script-plugin', '1.5.3')
        yaml_path = self.get_full_path('mock_blueprint/plugin_bad.yaml')
        zip_path = self.zip_files([tmp_file_path, yaml_path])
    ...
    @attr(client_min_version=3.1,
          client_max_version=base_test.LATEST_API_VERSION)
    def test_plugin_upload_with_inconsistent_with_wagon_yaml(self):
        tmp_file_path = self.create_wheel('cloudify-script-plugin', '1.5.3')
