...
class ExecutionSchedulesTestCase(BaseServerTestCase):
    ...
    def test_schedule_invalid_frequency(self):
        # TODO:: current scheme is too permissive!
        #  `10 doboshes` has passed, for instance`

        self.assertRaisesRegex(
            CloudifyClientError,
            '400: `10 vargas` is not a legal frequency expression.',
            self.client.execution_schedules.create,
            'bad-freq', self.deployment_id, 'install',
            since=self.an_hour_from_now, frequency='10 vargas'
        )
