...
Revision ID: 2f6caf12dfd0
Revises: 
Create Date: 2017-04-24 16:17:22.754610
...
revision = '2f6caf12dfd0'
down_revision = None
branch_labels = None
depends_on = None

snapshot_status = sa.Enum('created', 'failed', 'creating', 'uploaded', name='snapshot_status')
deployment_modification_status = sa.Enum('started', 'finished', 'rolledback', name='deployment_modification_status')
execution_status = sa.Enum('terminated', 'failed', 'cancelled', 'pending', 'started', 'cancelling', 'force_cancelling', name='execution_status')
action_type = sa.Enum('add', 'remove', 'modify', name='action_type')
entity_type = sa.Enum('node', 'relationship', 'property', 'operation', 'workflow', 'output', 'description', 'group', 'policy_type', 'policy_trigger', 'plugin', name='entity_type')
...
def upgrade():
    ...
    sa.Column('created_at', manager_rest.storage.models_base.UTCDateTime(), nullable=False),
    sa.Column('error', sa.Text(), nullable=True),
    sa.Column('is_system_workflow', sa.Boolean(name='is_system_workflow'), nullable=False),
    ...
    sa.Column('_storage_id', sa.Integer(), nullable=False),
    sa.Column('id', sa.Text(), nullable=True),
    sa.Column('timestamp', manager_rest.storage.models_base.LocalDateTime(), nullable=False, server_default=sa.func.now()),
