...
def install(ctx, **kwargs):
    for node in ctx.nodes:
        ...
        node.execute('cloudify.interfaces.lifecycle.create')
        node.set_state('created')
        node.set_state('configuring')
        node.execute('cloudify.interfaces.lifecycle.configure')
        node.set_state('configured')
        node.set_state('starting')
        node.execute('cloudify.interfaces.lifecycle.start')
...
def _host_post_start(ctx, host_node):
    ...
    if host_node.properties['install_agent'] is True:
        ...
        host_node.execute('cloudify.interfaces.worker_installer.install')
        host_node.execute('cloudify.interfaces.worker_installer.start')
        ...
        for plugin in host_node.plugins_to_install:
            ...
            host_node.execute('cloudify.interfaces.plugin_installer.install',
                              plugin=plugin)
        host_node.execute('cloudify.interfaces.worker_installer.restart')
