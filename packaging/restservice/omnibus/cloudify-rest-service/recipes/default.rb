...
when 'windows'
  execute 'bundle install' do
    cwd node['omnibus']['build_dir']
    command "#{node['omnibus']['build_dir']}\\load-omnibus-toolchain.bat & bundle install --without development"
  end
  execute 'build project' do
    cwd node['omnibus']['build_dir']
    command "#{node['omnibus']['build_dir']}\\load-omnibus-toolchain.bat & bundle exec omnibus build #{node['omnibus']['build_project']}"
  end
end
