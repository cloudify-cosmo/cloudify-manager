...
function set_manager_ip() {
  ...
  /usr/bin/sed -i -re "s#['\"]?amqp_host['\"]?: ['\"]?.*['\"]?#amqp_host: '${ip}'#" /opt/manager/cloudify-rest.conf
  /usr/bin/sed -i -re "s#['\"]?file_server_url['\"]?: ['\"]?https://[^:]*:([^']*)['\"]?#file_server_url: 'https://${ip}:\1'#" /opt/manager/cloudify-rest.conf
